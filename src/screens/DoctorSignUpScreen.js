...
const DoctorSignUpScreen = ({navigation}) => {
	...
	const signUpHandle = (data, position) => {
		...
		if(!checkEmpty(validData)){
	        ...
	        // // console.log("-------",params)
	        dispatch(loginActions.doctorRegistration(params))
		}else{
    ...
    const spec = useSelector(state => ({ ...state.doctorReducer}), shallowEqual)
    // // console.log('user',user)
    if(user.successMsg && check.submit){
        Alert.alert('Successfully Doctor Registration!', '', [
            {text: 'Close'}
        ]);
        navigation.goBack();
		...
		<Container>
          <Header style={style.appHeader}>
            ...
            <Body>
            	<Animatable.View animation="fadeInUp">
					<Text style={style.PageTitle}>{translations['Doctor']} {translations['Sign_Up']}</Text>
				</Animatable.View>
            </Body>
            <Right>
            </Right>
          ...
          <Content style={style.container}>
			...
			<View >
				...
				autoCapitalize="none"
				keyboardType="phone-pad"
				onChangeText={(expirience) => setData({...data,expirience})}
				...
				autoCapitalize="none"
				onChangeText={(about) => setData({...data,about})}
				value={data.about}
				/>
				{check.check_textInputChange ?
				<Animatable.View
				...
				*/}
				...
				<View style={style.dropdown}>
					...
					selectedValue={data.spec_id}
					mode="dialog"
					onValueChange={(spec_id) => setData({...data,spec_id})} >
						<Picker.Item label="Select Specialization" value="" />
						{spec.spList.length > 0 && spec.spList.map((row) => { 
